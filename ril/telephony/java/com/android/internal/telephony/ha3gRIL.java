...
public class ha3gRIL extends RIL implements CommandsInterface {
    ...
    public ha3gRIL(Context context, int networkMode,
	...
	Rlog.d(RILJ_LOG_TAG, "Test loop");
	    ...
	    if(checkifstatus(strings[i])){
	        String strOperatorName = Operators.operatorReplace(strings[i-2]);
	        Rlog.v(RILJ_LOG_TAG,
                   "XMM6360: Add OperatorInfo: " + strOperatorName +
                   ", " + strings[i-2] +
                   ", " + strings[i-1] +
                   ", " + strings[i]);
	        ret.add (new OperatorInfo(strOperatorName, strings[i-2], strings[i-1], "available"));
	    }
